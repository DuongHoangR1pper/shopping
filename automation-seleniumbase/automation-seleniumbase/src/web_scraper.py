...
class WebScraper:
    def __init__(self, username=WEB_USERNAME, password=WEB_PASSWORD, base_data_dir=BASE_DATA_DIR):
        self.categories = [
            "nha-cua-doi-song",
            "dien-thoai-phu-kien", 
            "dich-vu-phan-mem",
    ...
    def get_products(self, sb):
        products = sb.find_elements("css selector", ".rounded-b-md.bg-white.text-black")
        page_data = []
        
        for product in products:
            try:
                try: product_name = product.find_element("css selector", ".h-16.line-clamp-3").text.strip()
                except: product_name = "N/A"

                try: current_price = product.find_element("css selector", ".font-semibold.text-red-500").text.strip()
                except: current_price = "N/A"

                try: original_price = product.find_element("css selector", ".text-xs.text-gray-500.line-through").text.strip()
